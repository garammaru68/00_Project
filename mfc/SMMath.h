...
namespace TBASIS_EX
{
	...
	struct TVector2 : DirectX::XMFLOAT2
	{
		...
		static void Transform(const TVector2& v, const TDxMatrix& m, TVector2& result);
		static TVector2 Transform(const TVector2& v, const TDxMatrix& m);
		static void Transform(_In_reads_(count) const TVector2* varray, size_t count, const TDxMatrix& m, _Out_writes_(count) TVector2* resultArray);

		static void Transform(const TVector2& v, const TDxMatrix& m, TVector4& result);
		static void Transform(_In_reads_(count) const TVector2* varray, size_t count, const TDxMatrix& m, _Out_writes_(count) TVector4* resultArray);

		static void TransformNormal(const TVector2& v, const TDxMatrix& m, TVector2& result);
		static TVector2 TransformNormal(const TVector2& v, const TDxMatrix& m);
		static void TransformNormal(_In_reads_(count) const TVector2* varray, size_t count, const TDxMatrix& m, _Out_writes_(count) TVector2* resultArray);
	...
	struct TVector3 : DirectX::XMFLOAT3
	{
		...
		static void Transform(const TVector3& v, const TDxMatrix& m, TVector3& result);
		static TVector3 Transform(const TVector3& v, const TDxMatrix& m);
		static void Transform(_In_reads_(count) const TVector3* varray, size_t count, const TDxMatrix& m, _Out_writes_(count) TVector3* resultArray);

		static void Transform(const TVector3& v, const TDxMatrix& m, TVector4& result);
		static void Transform(_In_reads_(count) const TVector3* varray, size_t count, const TDxMatrix& m, _Out_writes_(count) TVector4* resultArray);

		static void TransformNormal(const TVector3& v, const TDxMatrix& m, TVector3& result);
		static TVector3 TransformNormal(const TVector3& v, const TDxMatrix& m);
		static void TransformNormal(_In_reads_(count) const TVector3* varray, size_t count, const TDxMatrix& m, _Out_writes_(count) TVector3* resultArray);
	...
	struct TVector4 : public XMFLOAT4
	{
		...
		static void Transform(const TVector4& v, const TDxMatrix& m, TVector4& result);
		static TVector4 Transform(const TVector4& v, const TDxMatrix& m);
		static void Transform(_In_reads_(count) const TVector4* varray, size_t count, const TDxMatrix& m, _Out_writes_(count) TVector4* resultArray);
	...
	//------------------------------------------------------------------------------
	// 4x4 TDxMatrix (assumes right-handed cooordinates)
	struct TDxMatrix : public XMFLOAT4X4
	{
		TDxMatrix() noexcept
			: XMFLOAT4X4(1.f, 0, 0, 0,
				...
				0, 0, 1.f, 0,
				0, 0, 0, 1.f) {}
		XM_CONSTEXPR TDxMatrix(float m00, float m01, float m02, float m03,
			...
			: XMFLOAT4X4(m00, m01, m02, m03,
				...
				m20, m21, m22, m23,
				m30, m31, m32, m33) {}
		explicit TDxMatrix(const TVector3& r0, const TVector3& r1, const TVector3& r2)
			: XMFLOAT4X4(r0.x, r0.y, r0.z, 0,
				r1.x, r1.y, r1.z, 0,
				r2.x, r2.y, r2.z, 0,
				0, 0, 0, 1.f) {}
		explicit TDxMatrix(const TVector4& r0, const TVector4& r1, const TVector4& r2, const TVector4& r3)
			: XMFLOAT4X4(r0.x, r0.y, r0.z, r0.w,
				r1.x, r1.y, r1.z, r1.w,
				r2.x, r2.y, r2.z, r2.w,
				r3.x, r3.y, r3.z, r3.w) {}
		TDxMatrix(const XMFLOAT4X4& M) { memcpy_s(this, sizeof(float) * 16, &M, sizeof(XMFLOAT4X4)); }
		TDxMatrix(const XMFLOAT3X3& M);
		TDxMatrix(const XMFLOAT4X3& M);

		explicit TDxMatrix(_In_reads_(16) const float *pArray) : XMFLOAT4X4(pArray) {}
		TDxMatrix(CXMMATRIX M) { XMStoreFloat4x4(this, M); }

		TDxMatrix(const TDxMatrix&) = default;
		TDxMatrix& operator=(const TDxMatrix&) = default;

		TDxMatrix(TDxMatrix&&) = default;
		TDxMatrix& operator=(TDxMatrix&&) = default;

		operator XMFLOAT4X4() const { return *this; }

		// Comparison operators
		bool operator == (const TDxMatrix& M) const;
		bool operator != (const TDxMatrix& M) const;

		// Assignment operators
		TDxMatrix& operator= (const XMFLOAT3X3& M);
		TDxMatrix& operator= (const XMFLOAT4X3& M);
		TDxMatrix& operator+= (const TDxMatrix& M);
		TDxMatrix& operator-= (const TDxMatrix& M);
		TDxMatrix& operator*= (const TDxMatrix& M);
		TDxMatrix& operator*= (float S);
		TDxMatrix& operator/= (float S);
		...
		// Unary operators
		TDxMatrix operator+ () const { return *this; }
		TDxMatrix operator- () const;
		...
		// TDxMatrix operations
		bool Decompose(TVector3& scale, TQuaternion& rotation, TVector3& translation);

		TDxMatrix Transpose() const;
		void Transpose(TDxMatrix& result) const;

		TDxMatrix Invert() const;
		void Invert(TDxMatrix& result) const;

		float Determinant() const;

		// Static functions
		static TDxMatrix CreateBillboard(const TVector3& object, const TVector3& cameraPosition, const TVector3& cameraUp, _In_opt_ const TVector3* cameraForward = nullptr);

		static TDxMatrix CreateConstrainedBillboard(const TVector3& object, const TVector3& cameraPosition, const TVector3& rotateAxis,
			_In_opt_ const TVector3* cameraForward = nullptr, _In_opt_ const TVector3* objectForward = nullptr);

		static TDxMatrix CreateTranslation(const TVector3& position);
		static TDxMatrix CreateTranslation(float x, float y, float z);

		static TDxMatrix CreateScale(const TVector3& scales);
		static TDxMatrix CreateScale(float xs, float ys, float zs);
		static TDxMatrix CreateScale(float scale);

		static TDxMatrix CreateRotationX(float radians);
		static TDxMatrix CreateRotationY(float radians);
		static TDxMatrix CreateRotationZ(float radians);

		static TDxMatrix CreateFromAxisAngle(const TVector3& axis, float angle);

		static TDxMatrix CreatePerspectiveFieldOfView(float fov, float aspectRatio, float nearPlane, float farPlane);
		static TDxMatrix CreatePerspective(float width, float height, float nearPlane, float farPlane);
		static TDxMatrix CreatePerspectiveOffCenter(float left, float right, float bottom, float top, float nearPlane, float farPlane);
		static TDxMatrix CreateOrthographic(float width, float height, float zNearPlane, float zFarPlane);
		static TDxMatrix CreateOrthographicOffCenter(float left, float right, float bottom, float top, float zNearPlane, float zFarPlane);

		static TDxMatrix CreateLookAt(const TVector3& position, const TVector3& target, const TVector3& up);
		static TDxMatrix CreateWorld(const TVector3& position, const TVector3& forward, const TVector3& up);

		static TDxMatrix CreateFromQuaternion(const TQuaternion& quat);

		static TDxMatrix CreateFromYawPitchRoll(float yaw, float pitch, float roll);

		static TDxMatrix CreateShadow(const TVector3& lightDir, const TPlane& plane);

		static TDxMatrix CreateReflection(const TPlane& plane);

		static void Lerp(const TDxMatrix& M1, const TDxMatrix& M2, float t, TDxMatrix& result);
		static TDxMatrix Lerp(const TDxMatrix& M1, const TDxMatrix& M2, float t);

		static void Transform(const TDxMatrix& M, const TQuaternion& rotation, TDxMatrix& result);
		static TDxMatrix Transform(const TDxMatrix& M, const TQuaternion& rotation);

		// Constants
		static const TDxMatrix Identity;
	};

	// Binary operators
	TDxMatrix operator+ (const TDxMatrix& M1, const TDxMatrix& M2);
	TDxMatrix operator- (const TDxMatrix& M1, const TDxMatrix& M2);
	TDxMatrix operator* (const TDxMatrix& M1, const TDxMatrix& M2);
	TDxMatrix operator* (const TDxMatrix& M, float S);
	TDxMatrix operator/ (const TDxMatrix& M, float S);
	TDxMatrix operator/ (const TDxMatrix& M1, const TDxMatrix& M2);
	// Element-wise divide
	TDxMatrix operator* (float S, const TDxMatrix& M);
	...
	struct TPlane : public XMFLOAT4
	{
		...
		// Static functions
		static void Transform(const TPlane& plane, const TDxMatrix& M, TPlane& result);
		static TPlane Transform(const TPlane& plane, const TDxMatrix& M);
	...
	struct TQuaternion : public XMFLOAT4
	{
		...
		static TQuaternion CreateFromRotationMatrix(const TDxMatrix& M);
	...
	public:
		...
		TVector3 Project(const TVector3& p, const TDxMatrix& proj, const TDxMatrix& view, const TDxMatrix& world) const;
		void Project(const TVector3& p, const TDxMatrix& proj, const TDxMatrix& view, const TDxMatrix& world, TVector3& result) const;

		TVector3 Unproject(const TVector3& p, const TDxMatrix& proj, const TDxMatrix& view, const TDxMatrix& world) const;
		void Unproject(const TVector3& p, const TDxMatrix& proj, const TDxMatrix& view, const TDxMatrix& world, TVector3& result) const;
	...
	static TVector4* D3DXVec2Transform(TVector4 *pOut, CONST TVector2 *pV, CONST TDxMatrix *pM)
	{
		return pOut;
	}

	// Transform (x, y, 0, 1) by matrix, project result back into w=1.
	static TVector2* D3DXVec2TransformCoord(TVector2 *pOut, CONST TVector2 *pV, CONST TDxMatrix *pM)
	{
		return pOut;
	}

	// Transform (x, y, 0, 0) by matrix.
	static TVector2* D3DXVec2TransformNormal(TVector2 *pOut, CONST TVector2 *pV, CONST TDxMatrix *pM)
	{
		return pOut;
	}

	// Transform Array (x, y, 0, 1) by matrix.
	static TVector4* D3DXVec2TransformArray(TVector4 *pOut, UINT OutStride, CONST TVector2 *pV, UINT VStride, CONST TDxMatrix *pM, UINT n)
	{
		return pOut;
	}

	// Transform Array (x, y, 0, 1) by matrix, project result back into w=1.
	static TVector2* D3DXVec2TransformCoordArray(TVector2 *pOut, UINT OutStride, CONST TVector2 *pV, UINT VStride, CONST TDxMatrix *pM, UINT n)
	{
		return pOut;
	}

	// Transform Array (x, y, 0, 0) by matrix.
	static TVector2* D3DXVec2TransformNormalArray(TVector2 *pOut, UINT OutStride, CONST TVector2 *pV, UINT VStride, CONST TDxMatrix *pM, UINT n)
	{
		return pOut;
	}
	//--------------------------
	...
	static TVector3* D3DXVec3TransformCoord(TVector3 *pOut, CONST TVector3 *pV, CONST TDxMatrix *pM)
	{
	...
	static TVector4* D3DXVec3Transform(TVector4 *pOut, CONST TVector3 *pV, CONST TDxMatrix *pM)
	{
		return pOut;
	}
	...
	static TVector3* D3DXVec3TransformNormal(TVector3 *pOut, CONST TVector3 *pV, CONST TDxMatrix *pM)
	{
		return pOut;
	}
	...
	static TVector4* D3DXVec3TransformArray(TVector4 *pOut, UINT OutStride, CONST TVector3 *pV, UINT VStride, CONST TDxMatrix *pM, UINT n)
	{
		return pOut;
	}

	// Transform Array (x, y, z, 1) by matrix, project result back into w=1.
	static TVector3* D3DXVec3TransformCoordArray(TVector3 *pOut, UINT OutStride, CONST TVector3 *pV, UINT VStride, CONST TDxMatrix *pM, UINT n)
	{
		return pOut;
	}
	...
	// non-affine matrix, the matrix you pass to this function should be the 
	// transpose of the inverse of the matrix you would use to transform a coord.
	static TVector3* D3DXVec3TransformNormalArray(TVector3 *pOut, UINT OutStride, CONST TVector3 *pV, UINT VStride, CONST TDxMatrix *pM, UINT n)
	{
		return pOut;
	}
	...
	static TVector3* D3DXVec3Project(TVector3 *pOut, CONST TVector3 *pV, CONST D3D10_VIEWPORT *pViewport,
		CONST TDxMatrix *pProjection, CONST TDxMatrix *pView, CONST TDxMatrix *pWorld)
	{
		return pOut;
	}
	...
	static TVector3* D3DXVec3ProjectArray(TVector3 *pOut, UINT OutStride, CONST TVector3 *pV, UINT VStride, CONST D3D10_VIEWPORT *pViewport,
		CONST TDxMatrix *pProjection, CONST TDxMatrix *pView, CONST TDxMatrix *pWorld, UINT n)
	{
		return pOut;
	}
	...
	static TVector4* D3DXVec4Transform(TVector4 *pOut, CONST TVector4 *pV, CONST TDxMatrix *pM)
	{
		return pOut;
	}

	// Transform vector array by matrix.
	static TVector4* D3DXVec4TransformArray(TVector4 *pOut, UINT OutStride, CONST TVector4 *pV, UINT VStride, CONST TDxMatrix *pM, UINT n)
	{
		return pOut;
	}


	static TDxMatrix* D3DXMatrixIdentity(TDxMatrix *pOut)
	{
		pOut->Identity(4, 4);
		return pOut;
	}
	static TDxMatrix* D3DXMatrixRotationQuaternion(TDxMatrix *pOut, CONST TQuaternion *pQ)
	{
		*pOut = TDxMatrix::CreateFromQuaternion(*pQ);
		return pOut;
	}
	static TQuaternion* D3DXQuaternionRotationMatrix(TQuaternion *pOut, CONST TDxMatrix *pM)
	{
		*pOut = TQuaternion::CreateFromRotationMatrix(*pM);
		return pOut;
	}

	static float D3DXMatrixDeterminant(CONST TDxMatrix *pM) {
		return pM->Determinant();
	}

	static HRESULT D3DXMatrixDecompose(TVector3 *pOutScale, TQuaternion *pOutRotation,
		TVector3 *pOutTranslation, TDxMatrix *pM)
	{
	...
	static TDxMatrix* D3DXMatrixTranspose(TDxMatrix *pOut, CONST TDxMatrix *pM) {
	...
	static TDxMatrix*  D3DXMatrixMultiply(TDxMatrix *pOut, CONST TDxMatrix *pM1, CONST TDxMatrix *pM2) {
		*pOut = (*pM1) * (*pM2);
		return pOut;
	}

	// Matrix multiplication, followed by a transpose. (Out = T(M1 * M2))
	static TDxMatrix*  D3DXMatrixMultiplyTranspose(TDxMatrix *pOut, CONST TDxMatrix *pM1, CONST TDxMatrix *pM2) {
	...
	// is non-NULL.
	static TDxMatrix*  D3DXMatrixInverse(TDxMatrix *pOut, float *pDeterminant, CONST TDxMatrix *pM) {
		pM->Invert(*pOut);
	...
	static TDxMatrix*  D3DXMatrixScaling(TDxMatrix *pOut, float sx, float sy, float sz) {
		*pOut = TDxMatrix::CreateScale(sx, sy, sz);
		return pOut;
	}

	// Build a matrix which translates by (x, y, z)
	static TDxMatrix*  D3DXMatrixTranslation(TDxMatrix *pOut, float x, float y, float z) {
		*pOut = TDxMatrix::CreateTranslation(x, y, z);
		return pOut;
	}

	// Build a matrix which rotates around the X axis
	static TDxMatrix*  D3DXMatrixRotationX(TDxMatrix *pOut, float Angle) {
		*pOut = TDxMatrix::CreateRotationX(Angle);
		return pOut;
	}

	// Build a matrix which rotates around the Y axis
	static TDxMatrix*  D3DXMatrixRotationY(TDxMatrix *pOut, float Angle) {
		*pOut = TDxMatrix::CreateRotationY(Angle);
		return pOut;
	}

	// Build a matrix which rotates around the Z axis
	static TDxMatrix*  D3DXMatrixRotationZ(TDxMatrix *pOut, float Angle) {
		*pOut = TDxMatrix::CreateRotationZ(Angle);
		return pOut;
	}

	// Build a matrix which rotates around an arbitrary axis
	static TDxMatrix*  D3DXMatrixRotationAxis(TDxMatrix *pOut, CONST TVector3 *pV, float Angle) {
		*pOut = TDxMatrix::CreateFromAxisAngle(*pV, Angle);
		return pOut;
	}

	// Yaw around the Y axis, a pitch around the X axis,
	// and a roll around the Z axis.
	static TDxMatrix*  D3DXMatrixRotationYawPitchRoll(TDxMatrix *pOut, float Yaw, float Pitch, float Roll) {
		*pOut = TDxMatrix::CreateFromYawPitchRoll(Yaw, Pitch, Roll);
		return pOut;
	}

	// Build transformation matrix.  NULL arguments are treated as identity.
	// Mout = Msc-1 * Msr-1 * Ms * Msr * Msc * Mrc-1 * Mr * Mrc * Mt
	static TDxMatrix*  D3DXMatrixTransformation(TDxMatrix *pOut, CONST TVector3 *pScalingCenter,
	...
	// Mout = Msc-1 * Msr-1 * Ms * Msr * Msc * Mrc-1 * Mr * Mrc * Mt
	static TDxMatrix*  D3DXMatrixTransformation2D(TDxMatrix *pOut, CONST TVector2* pScalingCenter,
	...
	static TDxMatrix*  D3DXMatrixAffineTransformation(TDxMatrix *pOut, float Scaling, CONST TVector3 *pRotationCenter,
	...
	// Mout = Ms * Mrc-1 * Mr * Mrc * Mt
	static TDxMatrix*  D3DXMatrixAffineTransformation2D(TDxMatrix *pOut, float Scaling, CONST TVector2* pRotationCenter,
		float Rotation, CONST TVector2* pTranslation) {
		return pOut;
	}

	// Build a lookat matrix. (right-handed)
	static TDxMatrix*  D3DXMatrixLookAtRH(TDxMatrix *pOut, CONST TVector3 *pEye, CONST TVector3 *pAt, CONST TVector3 *pUp) {
		return pOut;
	}

	// Build a lookat matrix. (left-handed)
	static TDxMatrix*  D3DXMatrixLookAtLH(TDxMatrix *pOut, CONST TVector3 *pEye, CONST TVector3 *pAt, CONST TVector3 *pUp) {
		*pOut = TDxMatrix::CreateLookAt(*pEye, *pAt, *pUp);
		return pOut;
	}

	// Build a perspective projection matrix. (right-handed)
	static TDxMatrix*  D3DXMatrixPerspectiveRH(TDxMatrix *pOut, float w, float h, float zn, float zf) {
		return pOut;
	}

	// Build a perspective projection matrix. (left-handed)
	static TDxMatrix*  D3DXMatrixPerspectiveLH(TDxMatrix *pOut, float w, float h, float zn, float zf) {
		*pOut = TDxMatrix::CreatePerspective(w, h, zn, zf);
		return pOut;
	}

	// Build a perspective projection matrix. (right-handed)
	static TDxMatrix*  D3DXMatrixPerspectiveFovRH(TDxMatrix *pOut, float fovy, float Aspect, float zn, float zf) {
		return pOut;
	}

	// Build a perspective projection matrix. (left-handed)
	static TDxMatrix*  D3DXMatrixPerspectiveFovLH(TDxMatrix *pOut, float fovy, float Aspect, float zn, float zf) {
		*pOut = TDxMatrix::CreatePerspectiveFieldOfView(fovy, Aspect, zn, zf);
		return pOut;
	}

	// Build a perspective projection matrix. (right-handed)
	static TDxMatrix*  D3DXMatrixPerspectiveOffCenterRH(TDxMatrix *pOut, float l, float r, float b, float t, float zn, float zf) {
		return pOut;
	}

	// Build a perspective projection matrix. (left-handed)
	static TDxMatrix*  D3DXMatrixPerspectiveOffCenterLH(TDxMatrix *pOut, float l, float r, float b, float t, float zn, float zf) {
		*pOut = TDxMatrix::CreatePerspectiveOffCenter(l, r, b, t, zn, zf);
		return pOut;
	}

	// Build an ortho projection matrix. (right-handed)
	static TDxMatrix*  D3DXMatrixOrthoRH(TDxMatrix *pOut, float w, float h, float zn, float zf) {
		return pOut;
	}

	// Build an ortho projection matrix. (left-handed)
	static TDxMatrix*  D3DXMatrixOrthoLH(TDxMatrix *pOut, float w, float h, float zn, float zf) {
		*pOut = TDxMatrix::CreateOrthographic(w, h, zn, zf);
		return pOut;
	}

	// Build an ortho projection matrix. (right-handed)
	static TDxMatrix*  D3DXMatrixOrthoOffCenterRH(TDxMatrix *pOut, float l, float r, float b, float t, float zn, float zf) {
		return pOut;
	}

	// Build an ortho projection matrix. (left-handed)
	static TDxMatrix*  D3DXMatrixOrthoOffCenterLH(TDxMatrix *pOut, float l, float r, float b, float t, float zn, float zf) {
		*pOut = TDxMatrix::CreateOrthographicOffCenter(l, r, b, t, zn, zf);
		return pOut;
	}

	// Build a matrix which flattens geometry into a plane, as if casting
	// a shadow from a light.
	static TDxMatrix*  D3DXMatrixShadow(TDxMatrix *pOut, CONST TVector4 *pLight, CONST TPlane *pPlane) {
		//*pOut = TDxMatrix::CreateShadow( *pLight, *pPlane);
		return pOut;
	}

	// Build a matrix which reflects the coordinate system about a plane
	static TDxMatrix*  D3DXMatrixReflect(TDxMatrix *pOut, CONST TPlane *pPlane) {
		*pOut = TDxMatrix::CreateReflection(*pPlane);
		return pOut;
	}
	...
	(CONST TQuaternion *pQ1, CONST TQuaternion *pQ2) {
		return 0.0f;
	};
	...
	// Expects unit quaternions.
	static TQuaternion* D3DXQuaternionSlerp(TQuaternion *pOut, CONST TQuaternion *pQ1,
		CONST TQuaternion *pQ2, float t) {
		return pOut;
	};
	...
	static TPlane* D3DXPlaneTransform(TPlane *pOut, CONST TPlane *pP, CONST TDxMatrix *pM) {
		return pOut;
	};
	// Transform an array of planes by a matrix.  The vectors (a,b,c) must be normal.
// M should be the inverse transpose of the transformation desired.
	static TPlane* D3DXPlaneTransformArray(TPlane *pOut, UINT OutStride, CONST TPlane *pP, UINT PStride, CONST TDxMatrix *pM, UINT n) {
		return pOut;
	};
...
namespace std
{
	...
	template<> struct less<TBASIS_EX::TDxMatrix>
	{
		bool operator()(const TBASIS_EX::TDxMatrix& M1, const TBASIS_EX::TDxMatrix& M2) const
		{
