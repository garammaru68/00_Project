...
#include "SScene.h"
#include "SWrite.h"
int SEdit::g_iControlID = 1000;
LRESULT	 SUserInterface::MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	for (auto iter : m_pChildObjects)
	{
		SUserInterface* pUI = (SUserInterface*)iter;
		int iRet = pUI->MsgProc(hWnd, msg, wParam, lParam);
		if (iRet >= 0) return iRet;
	}
	return -1;
}
SObject* SUserInterface::Clone()
{
	return new SUserInterface(*this);
}
bool  SUserInterface::PostFrame()
{
	return true;
}
SPoint  SActor::Gen(float x, float y)
{
	SPoint vRet(x, y);
	//// 0 ~ 800 - > 0 ~1 -> -1 ~ +1
	//vRet.x = x / 800;
...
void    SActor::GenCenter()
{
	...
	{
		SPoint vertex = m_VertexList[iV].p;
...
bool    SActor::SetData(float left, float top, float width, float height)
{
	m_rtInit.left = left;
...
{
	SPoint pos = Gen(m_rtInit.left, m_rtInit.top);
	m_VertexList[0].p = SPoint(pos.x, pos.y);
	m_VertexList[0].t = SPoint(0.0f, 0.0f);
	pos = Gen(m_rtInit.left + m_rtInit.right, m_rtInit.top);
	m_VertexList[1].p = SPoint(pos.x, pos.y);
	m_VertexList[1].t = SPoint(1.0f, 0.0f);
	pos = Gen(m_rtInit.left, m_rtInit.top + m_rtInit.bottom);
	m_VertexList[2].p = SPoint(pos.x, pos.y);
	m_VertexList[2].t = SPoint(0.0f, 1.0f);
	...
	m_VertexList[5].p = SPoint(pos.x, pos.y);
	m_VertexList[5].t = SPoint(1.0f, 1.0f);
	GenCenter();
}
bool    SDialogBox::SetData(RECT rt, float fScaleX0, float fScaleX1,
...
void  SDialogBox::Set(RECT rtSrc, RECT rtDesk)
{
	SObject::Set(rtSrc, rtDesk);
...
void  SDialogBox::Set(SPoint p, RECT rtSrc)
{
	SObject::Set(p, rtSrc);
	SetData(m_rtDesk, 0.1f, 0.1f,     // 화면 좌우 스케일 00, 02
		0.1f, 0.1f,		// 화면 상하 스케일 00, 20
...
LRESULT	 SEdit::MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
		...
		if (SCollision::RectInPt(g_rtClient, g_Input.GetPos()))
		{
			if (!SCollision::RectInPt(m_rtWndCtrl, g_Input.GetPos()))
			{
				::SetFocus(g_hWnd);
				m_bEditFocus = false;
				g_Input.SetEnable();
...
SObject* SEdit::Clone()
{
	return new SEdit(*this);
}
void   SEdit::CreateEdit()
{
