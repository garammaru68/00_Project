...
#include "STimer.h"
////////////////////////////////////////////////////////
HRESULT SDxWrite::CreateIndependentResource()
{
...
HRESULT SDxWrite::CreateDependentResource(
...
void SDxWrite::DeleteDependentResource()
{
	if (m_pBrush)
	{
		m_pBrush->Release();
...
bool SDxWrite::Set(HWND hWnd, int iWidth, int iHeight,
	IDXGISurface1* pSurface)
{
	// 디바이스 종속성, 디바이스 독립적
	m_iWidth = iWidth;
...
void SDxWrite::Draw(int x, int y, wstring msg, DWORD color)
{
	POINT p = { x, y };
	Draw(p, msg, color);
}
void SDxWrite::Draw(POINT pos, wstring msg, DWORD color)
{
	std::lock_guard<std::mutex> lock(m_hMutex);
