...
bool SModel::Frame()
{
	...
	{
		m_pObj->m_fTick = m_pObj->m_Scene.iFirstFrame * m_pObj->m_pCurrentScene->iTickPerFrame;
	}
	for (int iNode = 0; iNode < m_pObj->m_sNodeList.size(); iNode++)
	{
		...
		{
			if (pModelObject->animlist[iTick].iTick >= m_pObj->m_fTick)
			{
...
bool SModel::Render(ID3D11DeviceContext* pd3dContext)
{
	...
	//g_pImmediateContext->IASetIndexBuffer(m_pObj->m_pIndexBuffer, DXGI_FORMAT_R32_UINT, 0);
