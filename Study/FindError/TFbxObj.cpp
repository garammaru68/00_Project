	...
	for (int iPoly = 0; iPoly < iPolyCount; iPoly++)
	{
		...
		for (int iTriangle = 0; iTriangle < iTriangleCount; iTriangle++)
		{
			...
			for (int iIndex = 0; iIndex < 3; iIndex++)
			{
				...
				{
					TWeight* pW = &pObj->WeightList[iCornerIndices[iIndex]];					
					if (pW != nullptr)
					{
						//// size() 함수의 반환값이 size_t(부등호가 없는 정수)라서
						//// if문에서 절대값으로 비교된다?
						//size_t test = 4;
						//if (m_iMax < test)

						int WeightSize = pW->Weight.size();
						if (m_iMax < WeightSize)
