...
bool TImeUI::ImeUi_Initialize(HWND hwnd, bool bDisable)
{
	if (m_bInitialized)
	{
		return true;
...
void TImeUI::ImeUi_Uninitialize()
{
	if (!m_bInitialized)
	{
		return;
...
void TImeUI::Initialize(HWND hWnd)
{
	ImeUi_Initialize(hWnd);
}
//--------------------------------------------------------------------------------------
void TImeUI::Uninitialize()
{
	ImeUi_Uninitialize();
}
BOOL TImeUI::OnImeComposition(WPARAM wParam, LPARAM lParam)
{
	HIMC	hIMC;
...
void TImeUI::ResetCompositionString()
{
	m_CompString.clear();
}
int TImeUI::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
	{
		...
		case VK_BACK:
		{
			...
			{
				S_ITOR it = m_CompString.begin() + (m_CompString.size() - 1);
