...
void SControlUI::SetMatrix(Matrix* pWorld, Matrix* pView, Matrix* pProj)
{
	...
	{
		m_matWorld = *pWorld;
		m_pShape->m_vPos.x = pWorld->_41;
		m_pShape->m_vPos.y = pWorld->_42;
		m_pShape->m_vPos.z = pWorld->_43;
	}
	if (pView != NULL)
	{
...
HRESULT SControlUI::CreateResource(int iRectWidth, int iRectHeight)
{
	...
	//MatrixOrthoLH(&m_Projection[1], iRectWidth * 2, iRectHeight * 2, 0.0f, 1000.0f);
