...
SNode*  SQuadtree::CreateNode(SNode* pParentNode, UINT tl, UINT tr, UINT bl, UINT br)
{
	...
	// box
	pNode->m_Box.vMin = { 9999,9999,9999, };
	pNode->m_Box.vMax = { -9999,-9999,-9999, };
	for (DWORD dwIndex = 0; dwIndex < pNode->m_IndexList.size();
		dwIndex++)
	{
		Vector3 v = m_pMap->m_VertexList[pNode->m_IndexList[dwIndex]].p;
		if (v.x > pNode->m_Box.vMax.x)
		{
			pNode->m_Box.vMax.x = v.x;
		}
		if (v.y > pNode->m_Box.vMax.y)
		{
			pNode->m_Box.vMax.y = v.y;
		}
		if (v.z > pNode->m_Box.vMax.z)
		{
			pNode->m_Box.vMax.z = v.z;
		}

		if (v.x < pNode->m_Box.vMin.x)
		{
			pNode->m_Box.vMin.x = v.x;
		}
		if (v.y < pNode->m_Box.vMin.y)
		{
			pNode->m_Box.vMin.y = v.y;
		}
		if (v.z < pNode->m_Box.vMin.z)
		{
			pNode->m_Box.vMin.z = v.z;
		}
	}

	pNode->m_Box.vCenter = (pNode->m_Box.vMin + pNode->m_Box.vMax) / 2.0f;

	pNode->m_Sphere.vCenter = pNode->m_Box.vCenter;
	pNode->m_Sphere.fRadius =
		(pNode->m_Box.vMax - pNode->m_Box.vCenter).Length();
...
void SQuadtree::CalcPerVertexNormalsFastLookup(SNode* node)
{
	/*CalcFaceNormals();
...
void   SQuadtree::DrawNode(SNode* pNode,
	ID3D11DeviceContext* pd3dContext)
{
	if (pNode == nullptr) return;
